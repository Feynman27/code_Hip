...
void run(const std::string& period, TFile* outFile){
    ...
    std::vector<TH1F*> hDataPlusCent(nCentralityBins),hDataMinusCent(nCentralityBins);
    std::vector<TH1F*> hChAsymmCent(nCentralityBins);
    std::map<TString, float> m_nWPlus, m_nWPlusStatErr, m_nWMinus, m_nWMinusStatErr; 

    for(int icent=0; icent<nCentralityBins; ++icent){
        std::stringstream hName; 
        hName << "hData";
        if(doPeriodA) hName << "PerA";
        else if(doPeriodB) hName << "PerB"; 
        hName << "Centrality" << icent << "Plus" ;
        hDataPlusCent[icent] = new TH1F((hName.str()).c_str(),(hName.str()).c_str(),nEtaBins,xEtaBins);
        hDataPlusCent[icent] -> Sumw2();
        hName.str(std::string());
        hName << "hData";
        if(doPeriodA) hName << "PerA";
        else if(doPeriodB) hName << "PerB"; 
        hName << "Centrality" << icent << "Minus" ;
        hDataMinusCent[icent] = new TH1F( (hName.str()).c_str(),(hName.str()).c_str(),nEtaBins,xEtaBins);
        hDataMinusCent[icent] -> Sumw2();
        hName.str(std::string());
        hName << "hChAsymmCentrality" << icent;
        hChAsymmCent[icent] = new TH1F( (hName.str()).c_str(),(hName.str()).c_str(),nEtaBins,xEtaBins);
        hChAsymmCent[icent] -> Sumw2();
    }//icent
	...
	for(int ieta=0; ieta<nEtaBins; ++ieta){
	  ...
	  for(int icent=0; icent<nCentralityBins; ++icent){
          ...
          // container for yields in eta bin ieta, centrality bin icent
          m_nWPlus.insert(std::make_pair(namePlus,0) );
          m_nWPlusStatErr.insert(std::make_pair(namePlus,0) );
          m_nWMinus.insert(std::make_pair(nameMinus,0) );
          m_nWMinusStatErr.insert(std::make_pair(nameMinus,0) );
	    ...
	    for(int ipt=0; ipt<nPtBins; ++ipt){
            ...
            // differential in centrality
            // sum up yields in pT
            m_nWPlus[namePlus] += grPlusTemp->GetY()[ipt];
            m_nWPlusStatErr[namePlus] += TMath::Power(grPlusTemp->GetEYhigh()[ipt],2);
            ...
            // differential in centrality
            // sum up yields in pT
            m_nWMinus[nameMinus] += grMinusTemp->GetY()[ipt];
            m_nWMinusStatErr[nameMinus] += TMath::Power(grMinusTemp->GetEYhigh()[ipt],2);
        ...
        m_nWPlusStatErr[namePlus]=TMath::Sqrt(m_nWPlusStatErr[namePlus]);
        m_nWMinusStatErr[nameMinus]=TMath::Sqrt(m_nWMinusStatErr[nameMinus]);

        hDataPlusCent[icent]->SetBinContent(ieta+1, m_nWPlus[namePlus]/binW);
        hDataPlusCent[icent]->SetBinError(ieta+1,m_nWPlusStatErr[namePlus]/binW);

        hDataMinusCent[icent]->SetBinContent(ieta+1, m_nWMinus[nameMinus]/binW);
        hDataMinusCent[icent]->SetBinError(ieta+1,m_nWMinusStatErr[nameMinus]/binW);
	  } //icent
...
*/
    ...
    for(int icent=0; icent<nCentralityBins; ++icent){
      hChAsymmCent[icent] = getChargeAsymmetry(hDataPlusCent[icent],hDataMinusCent[icent]);  
	  outFile->cd();
      std::stringstream hName;
      hName << "hChAsymmPerABCentrality" << icent;
      hChAsymmCent[icent]->Write((hName.str()).c_str());
      delete hChAsymmCent[icent];
    }
