...
void checkName(TString checkName){

    if ( gROOT->FindObject(checkName) ) { 
        delete gROOT->FindObject(checkName);
    } 
}
void getWeightedMcHisto(RooDataSet* dset_pp_A,RooDataSet* dset_pn_A,RooDataSet* dset_pp_B, RooDataSet* dset_pn_B, TH1F* hReturn, RooRealVar& var, RooBinning& b){

    // Compents histos
	TH1F* h_pp_A=NULL, *h_pn_A=NULL;
	TH1F* h_pp_B=NULL, *h_pn_B=NULL;
	TH1F* h_wtd_A=NULL, *h_wtd_B=NULL;
    ...
    checkName ("h_wtd_A");
    checkName ("h_wtd_B");
    h_pp_A  = new TH1F("h_pp_A","temp",b.numBoundaries()-1,b.array());
    h_pn_A  = new TH1F("h_pn_A","temp",b.numBoundaries()-1,b.array());
    h_pp_B  = new TH1F("h_pp_B","temp",b.numBoundaries()-1,b.array());
    h_pn_B  = new TH1F("h_pn_B","temp",b.numBoundaries()-1,b.array());
    h_wtd_A = new TH1F("h_wtd_A","temp",b.numBoundaries()-1,b.array());
    h_wtd_B = new TH1F("h_wtd_B","temp",b.numBoundaries()-1,b.array());

	const float nProtons = 82.0;
	const float nNeutrons = 126.0;
	const float nNucleons = nProtons+nNeutrons;

    h_pp_A = (TH1F*)dset_pp_A->createHistogram("h_"+(TString)dset_pp_A->GetTitle(),var,Binning(b));
    h_pn_A = (TH1F*)dset_pn_A->createHistogram("h_"+(TString)dset_pn_A->GetTitle(),var,Binning(b));
    h_pp_B = (TH1F*)dset_pp_B->createHistogram("h_"+(TString)dset_pp_B->GetTitle(),var,Binning(b));
    h_pn_B = (TH1F*)dset_pn_B->createHistogram("h_"+(TString)dset_pn_B->GetTitle(),var,Binning(b));

    // weight by nucleons
    h_wtd_A->Add(h_pp_A,h_pn_A,nProtons/nNucleons,nNeutrons/nNucleons);
    h_wtd_B->Add(h_pp_B,h_pn_B,nProtons/nNucleons,nNeutrons/nNucleons);
    hReturn->Add(h_wtd_A,h_wtd_B);
...
RooDataSet* fillMuonDataSet(const TString& pathName, const TString& fileName, RooArgSet& muonArgSet, bool doPeriodA = false, bool doPeriodB=false, bool isMc=false){
    ...
    if(isMc==true){
        tree->SetBranchAddress("EF_mu8_matched",&_EF_mu8_matched);
    }
	// --- Set branch status ---
    ...
    for(int imu =0; imu < nmu; ++imu){
         ...
         tree->GetEntry(imu);
          ...
          // match trigger in MC
          if(isMc&&_EF_mu8_matched==0) continue;
...
void plot(TH1F* hCumulative, TH1F* hMcSig, TString sEta, TString sCent, TString sCharge, int charge){

    // Normalized signal to background-subtracted data
    hMcSig->Scale(hCumulative->Integral()/hMcSig->Integral());
    // check
    std::cout << "No. of entries in data: " << hCumulative->Integral() << std::endl;
    std::cout << "No. of entries in normalized MC: " << hMcSig->Integral() << std::endl;

    // scale by bin-width
	hCumulative->Scale(1.0,"width");
    hMcSig->Scale(1.0,"width");
	...
	hCumulative->Draw("pe");
    hMcSig->SetFillColor(kWhite);
    hMcSig->SetLineColor(kBlack);
    hMcSig->Draw("histsame");
    ...
    leg->SetBorderSize(0);
    leg->SetFillColor(0);
    leg->AddEntry(hCumulative,"Data","p");
    leg->AddEntry(hMcSig,"W#rightarrow#mu#nu","f");
    leg->Draw();
...
void WAnalysisHip(const std::string& period,bool doBackgroundSub=false, bool doCorrectYields=false){
    ...
    TString fileNameWmin_pp_A="",fileNameWmin_pn_A="",fileNameWmin_pp_B="",fileNameWmin_pn_B="";
    TString fileNameWplus_pp_A="",fileNameWplus_pn_A="",fileNameWplus_pp_B="",fileNameWplus_pn_B="";
	if(doPeriodA) fileNameDataIn += "A/HIpSingleMuonHardProbesDataPeriodA.09.18.2013";
        ...
        // mu+ Period A
        fileNameWplus_pp_A = "ntupleWplusmunu_pp.A.PowhegPythia8_AU2CT10.09.24.2014";
        fileNameWplus_pn_A = "ntupleWplusmunu_pn.A.PowhegPythia8_AU2CT10.09.24.2014";
        // mu+ Period B
        fileNameWplus_pp_B = "ntupleWplusmunu_pp.B.PowhegPythia8_AU2CT10.09.24.2014";
        fileNameWplus_pn_B = "ntupleWplusmunu_pn.B.PowhegPythia8_AU2CT10.09.24.2014";
    ...
    ////////////////
    // mu- Period A
    RooDataSet* mcWminSet_pp_A = fillMuonDataSet(baseString,fileNameWmin_pp_A+".root",muonArgSet,doPeriodA,doPeriodB,true);     
    mcWminSet_pp_A->Print(); 	
    RooDataSet* mcWminSet_pn_A = fillMuonDataSet(baseString,fileNameWmin_pn_A+".root",muonArgSet,doPeriodA,doPeriodB,true);     
    mcWminSet_pn_A->Print(); 	
    // mu- Period B
    RooDataSet* mcWminSet_pp_B = fillMuonDataSet(baseString,fileNameWmin_pp_B+".root",muonArgSet,doPeriodA,doPeriodB,true);     
    mcWminSet_pp_B->Print(); 	
    RooDataSet* mcWminSet_pn_B = fillMuonDataSet(baseString,fileNameWmin_pn_B+".root",muonArgSet,doPeriodA,doPeriodB,true);     
    mcWminSet_pn_B->Print(); 	

    // mu+ Period A
    RooDataSet* mcWplusSet_pp_A = fillMuonDataSet(baseString,fileNameWplus_pp_A+".root",muonArgSet,doPeriodA,doPeriodB,true);     
    mcWplusSet_pp_A->Print(); 	
    RooDataSet* mcWplusSet_pn_A = fillMuonDataSet(baseString,fileNameWplus_pn_A+".root",muonArgSet,doPeriodA,doPeriodB,true);     
    mcWplusSet_pn_A->Print(); 	
    // mu+ Period B
    RooDataSet* mcWplusSet_pp_B = fillMuonDataSet(baseString,fileNameWplus_pp_B+".root",muonArgSet,doPeriodA,doPeriodB,true);     
    mcWplusSet_pp_B->Print(); 	
    RooDataSet* mcWplusSet_pn_B = fillMuonDataSet(baseString,fileNameWplus_pn_B+".root",muonArgSet,doPeriodA,doPeriodB,true);     
    mcWplusSet_pn_B->Print(); 	
    ///////////////////
	...
	TH1F* hEtaCumulative_Wmin_wtd=NULL, *hEtaCumulative_Wplus_wtd=NULL;
    ...
    // MC W-->mu
  /*  hEtaCumulative_Wmin_pp_A  = new TH1F("hEtaCumulative_Wmin_pp_A","mu^{-}",bEta.numBoundaries()-1,bEta.array());
    hEtaCumulative_Wmin_pn_A  = new TH1F("hEtaCumulative_Wmin_pn_A","mu^{-}",bEta.numBoundaries()-1,bEta.array());
    hEtaCumulative_Wmin_pp_B  = new TH1F("hEtaCumulative_Wmin_pp_B","mu^{-}",bEta.numBoundaries()-1,bEta.array());
    hEtaCumulative_Wmin_pn_B  = new TH1F("hEtaCumulative_Wmin_pn_B","mu^{-}",bEta.numBoundaries()-1,bEta.array());
*/
    hEtaCumulative_Wmin_wtd  = new TH1F("hEtaCumulative_Wmin_wtd","mu^{-}",bEta.numBoundaries()-1,bEta.array());

   /* hEtaCumulative_Wplus_pp_A  = new TH1F("hEtaCumulative_Wplus_pp_A","mu^{+}",bEta.numBoundaries()-1,bEta.array());
    hEtaCumulative_Wplus_pn_A  = new TH1F("hEtaCumulative_Wplus_pn_A","mu^{+}",bEta.numBoundaries()-1,bEta.array());
    hEtaCumulative_Wplus_pp_B  = new TH1F("hEtaCumulative_Wplus_pp_B","mu^{+}",bEta.numBoundaries()-1,bEta.array());
    hEtaCumulative_Wplus_pn_B  = new TH1F("hEtaCumulative_Wplus_pn_B","mu^{+}",bEta.numBoundaries()-1,bEta.array());
*/
    hEtaCumulative_Wplus_wtd  = new TH1F("hEtaCumulative_Wplus_wtd","mu^{+}",bEta.numBoundaries()-1,bEta.array());

    // Fill integrated MC histograms
    // W-->mu
    // mu-
    getWeightedMcHisto(mcWminSet_pp_A,mcWminSet_pn_A,mcWminSet_pp_B,mcWminSet_pn_B,hEtaCumulative_Wmin_wtd,muonEta,bEta);
    hEtaCumulative_Wmin_wtd->SetName("hEtaCumulativeMinus");
    std::cout << "No. of W^- --> mu^- events in weighted MC: " << hEtaCumulative_Wmin_wtd->GetEntries() << std::endl;
    getWeightedMcHisto(mcWplusSet_pp_A,mcWplusSet_pn_A,mcWplusSet_pp_B,mcWplusSet_pn_B,hEtaCumulative_Wplus_wtd,muonEta,bEta);
    hEtaCumulative_Wplus_wtd->SetName("hEtaCumulativePlus");
    std::cout << "No. of W^+ --> mu^+ events in weighted MC: " << hEtaCumulative_Wplus_wtd->GetEntries() << std::endl;

    /*hEtaCumulative_Wmin_pp_A = (TH1F*)mcWminSet_pp_A->createHistogram("h_"+(TString)hEtaCumulative_Wmin_pp_A->GetTitle(),muonEta,Binning(bEta));
    hEtaCumulative_Wmin_pn_A = (TH1F*)mcWminSet_pn_A->createHistogram("h_"+(TString)hEtaCumulative_Wmin_pn_A->GetTitle(),muonEta,Binning(bEta));
    hEtaCumulative_Wmin_pp_B = (TH1F*)mcWminSet_pp_B->createHistogram("h_"+(TString)hEtaCumulative_Wmin_pp_B->GetTitle(),muonEta,Binning(bEta));
    hEtaCumulative_Wmin_pn_B = (TH1F*)mcWminSet_pn_B->createHistogram("h_"+(TString)hEtaCumulative_Wmin_pn_B->GetTitle(),muonEta,Binning(bEta));
    */
    // weight by nucleons
    /*hEtaCumulative_Wmin_wtd_A->Add(hEtaCumulative_Wmin_pp_A,hEtaCumulative_Wmin_pn_A,nProtons/nNucleons,nNeutrons/nNucleons);
    hEtaCumulative_Wmin_wtd_B->Add(hEtaCumulative_Wmin_pp_B,hEtaCumulative_Wmin_pn_B,nProtons/nNucleons,nNeutrons/nNucleons);
    hEtaCumulative_Wmin_wtd->Add(hEtaCumulative_Wmin_wtd_A,hEtaCumulative_Wmin_wtd_B);
    */
    // mu+
    /*hEtaCumulative_Wplus_pp_A = (TH1F*)mcWplusSet_pp_A->createHistogram("h_"+(TString)hEtaCumulative_Wplus_pp_A->GetTitle(),muonEta,Binning(bEta));
    hEtaCumulative_Wplus_pn_A = (TH1F*)mcWplusSet_pn_A->createHistogram("h_"+(TString)hEtaCumulative_Wplus_pn_A->GetTitle(),muonEta,Binning(bEta));
    hEtaCumulative_Wplus_pp_B = (TH1F*)mcWplusSet_pp_B->createHistogram("h_"+(TString)hEtaCumulative_Wplus_pp_B->GetTitle(),muonEta,Binning(bEta));
    hEtaCumulative_Wplus_pn_B = (TH1F*)mcWplusSet_pn_B->createHistogram("h_"+(TString)hEtaCumulative_Wplus_pn_B->GetTitle(),muonEta,Binning(bEta));
   */ 
	...
	plot(hEtaCumulativePlus,hEtaCumulative_Wplus_wtd, sEta,sCent,"#mu^{+}",+1);
	plot(hEtaCumulativeMinus,hEtaCumulative_Wmin_wtd, sEta,sCent,"#mu^{-}",-1);

	// Save yields to outFile
	result.write(outFile,1);
	result.write(outFile,-1);

    delete hEtaCumulativePlus;
    delete hEtaCumulativeMinus;
    delete hEtaCumulative_Wmin_wtd;
    delete hEtaCumulative_Wplus_wtd;
